...
def controlLoop():
    ...
    #endregion
    dsegment = np.linalg.norm(np.diff(waypointSequence,axis=1),axis=0)
    s = np.concatenate(([0.0],np.cumsum(dsegment)))
    S = s[-1]
    print('Cumulative length of path')
    print(S)
